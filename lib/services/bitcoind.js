...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  if (self._getNetworkOption()) {
    ...
    var bitcoinStartTimeout = parseInt(process.env.LITECOIND_START_TIMEOUT) || 300;
    var times = Math.round(bitcoinStartTimeout * 1000 / self.startRetryInterval) || 1;

    async.retry({times: times, interval: self.startRetryInterval}, function(done) {
      if (self.node.stopping) {
        exitShutdown = true;
        return done();
      }
      ...
      node.client = new BitcoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
        port: self.spawn.config.rpcport || 9332,
